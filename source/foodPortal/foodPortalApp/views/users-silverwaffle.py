...
def index(request):
    ...
    if form.is_valid():
        ...
        user = Member.objects.create_user(form.data['username'],form.data['email'],form.data['password'],
                                          ...
                                          note=form.data['note'],first_name=form.data['first_name'],
...
def home(request):
    ...
    try:
        order = Order.objects.get(customer__id=currUser.id)
        orderItems = order.orderItems.all()
        menu = orderItems[0].menu
    except:
        order = None
        orderItems = None
        menu = None
    try:
        myMenus = Menu.objects.all().filter(Q(curator_id=currUser.id))
        numMenus = len(myMenus)
    except ObjectDoesNotExist:
        numMenus = 0
    try:
        myClosedOrders = Order.objects.filter(Q(customer_id=currUser.id) and Q(status="Closed"))
